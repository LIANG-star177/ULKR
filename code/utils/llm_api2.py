...
def chatglm_response(text_list):
    tokenizer = AutoTokenizer.from_pretrained("/data/apps/lacode/self-correct-retriever/baseline/ChatGLM-6B-main/chatglm-6b", trust_remote_code=True)
    model = AutoModel.from_pretrained("/data/apps/lacode/self-correct-retriever/baseline/ChatGLM-6B-main/chatglm-6b", trust_remote_code=True).half().cuda()
    model = model.eval()
...
def few_shot(query, in_context_contents,config):
    new_query = []
    for i in range(len(query)):
        new_context = []
        if config.task=="ljp":
            query_one = query[i]
            for text in in_context_contents[i]:
                last_index = text.rfind("根据法条第")
                before_text = text[:last_index]
                after_text = text[last_index:]            
                before_text = cut_sentences(before_text,400)
                new_context.append("违法事实: "+before_text+"判决: "+after_text)
            new_query.append("\n".join(new_context)+"违法事实: "+cut_sentences(query_one,400)+"判决: ")
        if config.task=="cvg":
            query_one = query[i].replace("：\n\n","").replace("\n\n","")
            for text in in_context_contents[i]:
                text = text.replace("：\n\n","").replace("\n\n","")
                new_context.append(cut_sentences(text,400))
            new_query.append(config.curr_prompt+"\n".join(new_context)+"\n违法事实: "+cut_sentences(query_one,400)+"法院观点: ")
            # new_query.append(config.curr_prompt+"\n违法事实: "+cut_sentences(query_one,400)+"法院观点: ")
        if config.task =="Consult":
            for text in in_context_contents[i]:
                new_context.append(cut_sentences(text,400))
            new_query.append(config.curr_prompt+"\n".join(new_context)+"\n问题: "+cut_sentences(query[i],400)+"答案: ")
            # new_query.append(config.curr_prompt+"问题: "+cut_sentences(query[i],400)+"答案: ")
    return new_query
...
def get_llm_response(query, in_context_contents=None, combine_way="answer", config = None):
    ...
    if in_context_contents:
        query = few_shot(query, in_context_contents,config)
...
def get_openai_response(query, in_context_contents=None, combine_way=None, config = None):
    response_function = llm_response_function[config.mlm_api]
    prompt = few_shot(query, in_context_contents,config)
    combine_labels, droped_knowledge, combined_knowledge=[], [], []
    i = 0
    batch = 1 if config.mlm_api=="gpt-3.5-turbo" else 4
    data_all = []
    while i < len(query):
        responses = []
        ...
        try:
            ...
            for run in range(len(prompt_list)):
                if config.mlm_api=="gpt-3.5-turbo":
                    resp = resp_temp["choices"][run]["message"]["content"]#3.5
                if config.mlm_api=="text-davinci-003" or config.mlm_api=="text-davinci-002":
