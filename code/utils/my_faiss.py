...
class FAISS(VectorStore):
    ...
    def __from(
        ...
        **kwargs: Any,
        ...
        try:
            embed_query = embedding.module.embed_query
        except:
    ...
    def from_documents(
        ...
        **kwargs: Any,
        ...
        texts_all = [d.page_content for d in documents]
        metadatas_all = [d.metadata for d in documents]
        try:
            bert = embedding.module.bert
            gnn = embedding.module.gnn
            fc = embedding.module.fc
        except:
            bert = embedding.bert
            gnn = embedding.gnn
            fc = embedding.fc       

        tokenizer = BertTokenizer.from_pretrained('/root/data1/liang/self-correct-retriever/checkpoint/m3e-base')
        all_embeddings = []
        batch_size = 64  # 设置批处理大小
        # length_sorted_idx = np.argsort([-_text_length(sen) for sen in texts_all])
        # texts_all = [texts_all[idx] for idx in length_sorted_idx]
        # metadatas_all = [metadatas_all[idx] for idx in length_sorted_idx]

        for i in tqdm(range(0, len(texts_all), batch_size)):
            texts = texts_all[i:i+batch_size]
            metadatas = metadatas_all[i:i+batch_size]
        # for i in tqdm(range(len(texts_all))):
        #     texts = texts_all[i]
        #     metadatas = metadatas_all[i]
            with torch.no_grad():
                entailment = tokenizer(texts, max_length=512,truncation=True, padding='max_length', return_tensors='pt').to(device)
                entailment = bert(**entailment, output_hidden_states=True).last_hidden_state[:, 0]

                entailment_key = [data["key"] for data in metadatas]
                entailment_key = tokenizer(entailment_key, max_length=512,truncation=True, padding='max_length', return_tensors='pt').to(device)
                entailment_key = bert(**entailment_key, output_hidden_states=True).last_hidden_state[:, 0]

                entailment_path = [data["path"] for data in metadatas]
                entailment_path = tokenizer(entailment_path, max_length=512,truncation=True, padding='max_length', return_tensors='pt').to(device)
                entailment_path = bert(**entailment_path, output_hidden_states=True).last_hidden_state[:, 0]

                entailment_nodes = []
                for data in metadatas:
                    entailment_nodes.extend(data["node_id"])
                entailment_sub_g = graph.subgraph(entailment_nodes)
                # gnn
                entailment_fusion = gnn(entailment_sub_g,entailment,entailment_key,entailment_path)
                ...
                all_embeddings.extend(entailment_fusion)
                torch_gc()
        # all_embeddings = [all_embeddings[idx] for idx in np.argsort(length_sorted_idx)]
        embeddings = np.asarray([emb.cpu().numpy() for emb in all_embeddings])
        # embeddings = embedding.embed_documents(texts)
        return cls.__from(
            texts_all,
            embeddings,
    ...
    def load_local(
        ...
        **kwargs: Any,
        ...
        try:
            embed_query = embeddings.module.embed_query
        except:
            embed_query = embeddings.embed_query
        return cls(
            embed_query,index, docstore, index_to_docstore_id, **kwargs
        )
    ...
    def deserialize_from_bytes(
        ...
        **kwargs: Any,
        ...
        return cls(
            embeddings.module.embed_query, index, docstore, index_to_docstore_id, **kwargs
        )
