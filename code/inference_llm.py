...
class testCFG:
    ...
    mlm_api = "text-davinci-003"
    retriever = "sailer"
    top_k = 2
    test_path_dic = {"article":["请问下面案件中被告人触犯的具体法律是哪一条？"],
              "charge":["请问下面案件中被告人犯的是什么罪？"],
              "penalty":["请预测下面案件中被告人可能的刑期是多少个月？"],
              "cvg":"/root/data1/liang/self-correct-retriever/data/knowledge_base/test_cvg.json",
              "ljp":"/root/data1/liang/self-correct-retriever/data/knowledge_base/single_test_ljp.json",
              "Question Answering":"/root/data1/liang/self-correct-retriever/baseline/LawBench-main/data/zero_shot/1-2.json",
              "Consult":"/root/data1/liang/self-correct-retriever/data/knowledge_base/test_consult.json"}

    know_path_dic = {"ljp":"/root/data1/liang/self-correct-retriever/data/hera_knowbase3/类案",
                     "cvg":"/root/data1/liang/self-correct-retriever/data/hera_knowbase3/法院观点"

    }
    retrieval_model_dic = {"contriever":'/root/data1/liang/self-correct-retriever/checkpoint/m3e-base',
                           "ulr":"/root/data1/liang/self-correct-retriever/saved_model/simcse_cvg_4_2650.pt",
                           "bm25":None,
                           "sailer":"/root/data1/liang/self-correct-retriever/baseline/SAILER/checkpoint-60000"
    }
    vector_store_dic = {"contriever":None,
                        "ulr":"/root/data1/liang/self-correct-retriever/data/hera_knowbase3/法院观点",
                        "bm25":None,
                        "sailer":None,
    }
    test_num = 100
...
def write_log(data_all,config):
    with open("/root/data1/liang/self-correct-retriever/ulr_result/"+config.retriever+"_"+config.task+"+retrieval_{}.json", "w", encoding="utf-8") as f:
...
def init_vector_store(file_path, model_path, vector_path = None, retriever= "bm25", top_k=None, device= None):
    ...
    if retriever=="ulr":
        init_path = "/root/data1/liang/self-correct-retriever/checkpoint/m3e-base"
...
def test(config):
    ...
    with open(config.test_path_dic[config.task],"r",encoding="utf-8") as f:
        if config.task in ["Article Recitation","Question Answering","Consult"]:
            ...
            for item in tqdm(items[:config.test_num]):
                ...
                in_context_content = search(item["question"], vector_store, config.top_k, local_doc_qa, config.retriever)
    ...
    with open("/root/data1/liang/self-correct-retriever/ulr_result/"+config.retriever+"_"+config.mlm_api+"_{}.txt".format(config.task), "w", encoding='utf-8') as f:
        for dic in results:
