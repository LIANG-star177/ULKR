...
class testCFG:
    ...
    test_path_dic = {"article":["请问下面案件中被告人触犯的具体法律是哪一条？"],
              "charge":["请问下面案件中被告人犯的是什么罪？"],
              "penalty":["请预测下面案件中被告人可能的刑期是多少个月？"],
              "cvg":"/root/data1/liang/self-correct-retriever/data/new_cvg_test.json",
              "Article Recitation":"/root/data1/liang/self-correct-retriever/baseline/LawBench-main/data/zero_shot/1-1.json",
              "Question Answering":"/root/data1/liang/self-correct-retriever/baseline/LawBench-main/data/zero_shot/1-2.json",
              "Consult":"/root/data1/liang/self-correct-retriever/baseline/LawBench-main/data/zero_shot/3-8.json"}
    test_num = 500
    self_correct = False  
...
def test(config):
    ...
    with open("/root/data1/liang/self-correct-retriever/test_result/"+name_add+"_"+config.mlm_api+"_generation_{}.txt".format(config.task), "w", encoding='utf-8') as f:
        for dic in results:
