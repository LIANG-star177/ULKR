...
class testCFG:
    ...
    count_dic = {"can_not_combine":0,
                "need_extra":0}
    task = "cvg"
    test_path = "/data/apps/lacode/self-correct-retriever/data/new_cvg_test.json"
    self_correct = True  
...
def test(config):
    ...
    with open("/data/apps/lacode/self-correct-retriever/test_result/"+name_add+"re_generation_{}.txt".format(config.task), "w") as f:
        for dic in results:
